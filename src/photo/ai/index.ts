...
export const getAiImageQuery = (
  ...
  const queryText = AI_IMAGE_QUERIES[query];
  
  if (query === 'tags' && existingTags.length > 0) {
    const tags = existingTags.map(({ tag }) => tag).join(', ');
    return tags
      ? `${queryText}. Consider using some of these existing tags, but only if they are relevant: ${tags}.`
      : queryText;
  }
