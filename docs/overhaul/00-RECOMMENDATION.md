...
5. **Confirm Supabase isn't auto-pausing** and the Vercel **function region == `iad1`/us-east-1** (co-located with the DB). A paused free-tier project or a region mismatch makes first/dynamic requests very slow. (No `vercel.json` → region is the dashboard default; verify it.)
...
- Vercel project **function region** (dashboard) == us-east-1/iad1?
