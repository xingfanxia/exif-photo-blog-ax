...
> **Deferred (needs live DB + data):** `SELECT indexname FROM pg_indexes` + `EXPLAIN ANALYZE` index-scan oracle — an empty table seq-scans regardless. Runs at bootstrap (`npm run db:migrate` creates them) + after photos exist. database-reviewer pass done.
